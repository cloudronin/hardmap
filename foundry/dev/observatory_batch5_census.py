...
OUT = LAT / "observatory_batch5_census.json"
...
FAMILY_RAMP = {
    ...
    "string":       ("alphabet size at fixed string length (amended by erratum 2026-07-27)", (2, 6)),
}

# row → (family, what this row instantiates the family ramp AS, declared structural expectation)
ROSTER = {
    "minimum-common-string-partition": ("string", "alphabet size at fixed string length",
                                        "upward_closed"),
    "bin-packing":            ("optimization", "items per bin capacity", "downward_closed"),
    "k-set-packing":          ("optimization", "sets per universe element", "downward_closed"),
    "min-sum-set-cover":      ("optimization", "sets per universe element", "upward_closed"),
    "job-interval-selection": ("optimization", "jobs per time slot", "downward_closed"),
    "cutwidth":               ("graph", "edge density of the ground graph", None),
    "domatic-number":         ("graph", "edge density of the ground graph", "upward_closed"),
    "biclique-cover":         ("graph", "edge density of the ground graph", "upward_closed"),
}

# MCSP is the first CONTRAST-DIAL row. Its ramp is two declared levels, not a trajectory.
CAPTURE_MODE = {"minimum-common-string-partition": "CONTRAST-DIAL"}

FLAGGED = {
    "covering-radius": {
        "disposition": "STILL HELD — needs a framing that keeps the ambient fixed",
        "why": ("carried forward from batch 4 unchanged. The algebraic family's last unbuilt row still "
                "has no region formulation whose ambient stays put while the dial moves."),
    },
}
...
def main() -> int:
    ...
    doc = {
        ...
        "batch": BATCH,
        "why_this_batch": (
            "batch 5 exercises the CONTRAST-DIAL path for the first time. "
            "minimum-common-string-partition re-enters under the typing ruled on 2026-07-27 — two "
            "declared alphabet levels rather than a ramp — and seven ramped rows keep the fan-out "
            "moving and the frontier growing toward wave 1's first adjudicable candidate."),
