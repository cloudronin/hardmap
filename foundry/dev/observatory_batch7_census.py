...
OUT = LAT / "observatory_batch7_census.json"
...
# Vetted before hashing against the LEXICON v2 verified-clean queue. No number-theoretic component:
# that family's four REACH-subset rows are all built, which is why wave 3's held candidate is
# HELD-path-gated rather than HELD-power (Ruling 2).
ROSTER = {
    "total-dominating-set":     ("graph", "edge density of the ground graph", "upward_closed"),
    "power-dominating-set":     ("graph", "edge density of the ground graph", "upward_closed"),
    "triangle-packing":         ("graph", "edge density of the ground graph", "downward_closed"),
    "cycle-packing":            ("graph", "edge density of the ground graph", "downward_closed"),
    "planar-vertex-deletion":   ("graph", "edge density of the ground grid graph", "upward_closed"),
    "lex-first-maximal-independent-set": ("graph", "edge density of the ground graph", None),
    "quadratic-knapsack":       ("optimization", "capacity constraints per item", "downward_closed"),
    "k-minimum-spanning-tree":  ("graph", "edge density of the ground graph", None),
}
CAPTURE_MODE = {}
FLAGGED = {
    "covering-radius": {"disposition": "TYPED OUT — deferred-no-ambient-stable-framing (Ruling 4)",
                        "why": "no longer carried; re-entry route recorded in the maptrail"},
}
...
def main() -> int:
    ...
    doc = {
        ...
        "batch": BATCH,
        "why_this_batch": (
            "drawn from the LEXICON v2 verified-clean queue (coverage 32.3% -> 53.5%) and "
            "vetted before hashing. Eight vertex- and item-subset rows, every ambient fixed by "
            "construction. Its reservation takes the frontier from 8 to 10 clusters, which clears "
            "the four candidates sitting one row from adjudication at wave 3. NO number-theoretic "
            "component: that family's subset rows are exhausted, which is what makes wave 3's slated "
            "candidate HELD-path-gated rather than HELD-power."),
