...
OUT = LAT / "observatory_batch6_census.json"
...
# EVERY ROW HERE WAS VETTED BEFORE THIS ROSTER WAS HASHED (roster protocol, 2026-07-27): each appears in
# `reach_subset_readjudication.json`'s verified-clean queue, meaning its own canonical_encoding names a
# SUBSET certificate under the sealed lexicon. Batch 5's three mis-typings are what this check exists for.
ROSTER = {
    "directed-feedback-vertex-set": ("graph", "edge density of the ground digraph", "upward_closed"),
    "maximum-minimal-vertex-cover": ("graph", "edge density of the ground graph", None),
    "partial-vertex-cover":         ("graph", "edge density of the ground graph", "upward_closed"),
    "planar-dominating-set":        ("graph", "edge density of the ground grid graph", "upward_closed"),
    "planar-independent-set":       ("graph", "edge density of the ground grid graph", "downward_closed"),
    "planar-vertex-cover":          ("graph", "edge density of the ground grid graph", "upward_closed"),
    "multidimensional-knapsack":    ("optimization", "capacity constraints per item", "downward_closed"),
    "red-blue-set-cover":           ("optimization", "blue elements per candidate set", "upward_closed"),
}
CAPTURE_MODE = {}
FLAGGED = {
    "covering-radius": {"disposition": "STILL HELD — needs an ambient-fixed framing",
                        "why": "carried forward unchanged from batches 4 and 5"},
}
...
def main() -> int:
    ...
    doc = {
        ...
        "batch": BATCH,
        "why_this_batch": (
            "the first roster drawn from the VERIFIED-CLEAN queue produced by the "
            "REACH-subset re-adjudication, and the first vetted before hashing. Eight vertex- and "
            "item-subset rows, every ambient fixed by construction. Its reservation takes the "
            "frontier from 6 to 8 clusters — the size at which wave 1's nearest held candidate "
            "becomes adjudicable."),
