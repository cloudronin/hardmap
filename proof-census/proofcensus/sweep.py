...
def sample_k_parallel(cnf: CNF, sampler: str, K: int, *, seed: int = 0, s1_budget: int = 4000,
                      ...
                      max_attempts: int | None = None, executor=None) -> SampleResult:
    """Collect ``K`` verified refutations of ``cnf`` under ``sampler`` across worker processes.

    Over-submits batches of seeds and drains results until ``K`` verified are gathered. Accounting matches
    :func:`proofcensus.sample.sample_k` (verified / verify-discard / budget-exceeded). Pass a shared
    ``executor`` (a ``ProcessPoolExecutor``) to avoid re-spawning a pool per call across a sweep.
    """
    ...
    ex = executor or ProcessPoolExecutor(max_workers=n_workers)
    try:
        while res.n_verified < K and res.n_attempts < cap:
            need = K - res.n_verified
            # Cap the in-flight wave at 4×workers so results (which can be long step-lists at n=60) don't
            # all materialize at once — bounds peak memory (a BrokenProcessPool from OOM took down a run).
            batch = min(cap - res.n_attempts, max(need, n_workers), 4 * n_workers)
